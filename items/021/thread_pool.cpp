...
template <typename RAIter> int parallel_sum(RAIter begin, RAIter end) {
  auto len = end - begin;
  ...
  auto handle =
      pool.queue([mid, end]() { return parallel_sum<RAIter>(mid, end); });
...
//     auto future = pool.queue([i]() { return i; });
